Feature: PMobile Settings feature
  ...
  Scenario: Pmobile Font Size Change - Normal
    ...
    And  Driver checks that the normal radio button is selected
    ...
    #TC -740
  Scenario: PMobile Font Change Selection Presents A Reboot GUF and driver taps No from the Alert pop-up
    Given Driver taps the Font section
    ...
    #TC -740
  Scenario: PMobile Font Change Selection Presents A Reboot GUF and the driver allows the GUF to timeout
    Given Driver taps the Font section
    When Driver taps the large radio button
